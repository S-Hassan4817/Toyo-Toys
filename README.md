# Toyo Toys - E-Project

## Project Overview
This project is a professional frontend website developed for **Toyo Toys**, a toy company. It was created as part of my **DISM (Diploma In Information System Management)** at **Aptech**.

The goal of this project was to design a responsive, user-friendly interface that showcases products effectively and provides a modern shopping experience for customers.

## Tech Stack Used
* **HTML5:** For structured and semantic web content.
* **CSS3:** For custom styling and layout design.
* **Bootstrap 5:** For a fully responsive grid system and pre-built UI components.
* **JavaScript:** Used for basic interactivity and UI logic.

## Key Features
* Responsive design (Works on Mobile, Tablet, and Desktop).
* Navigation bar with smooth scrolling.
* Product display sections.
* Contact Us page.

---

## 👨‍💻 About the Developer
My name is **Hassan Ali**, a 17-year-old passionate Software Developer currently building my foundation in Computer Science.

* **Academic Path:** Currently in **HSSC Part 2 (ICS)**, specializing in Computer Science.
* **Professional Training:** Pursuing a **Diploma in Information System Management (DISM)** at Aptech Pakistan.
* **Goal:** My mission is to become a Software Engineer. After my HSSC and DISM, I plan to pursue a formal **BS in Computer Science** to further refine my programming expertise.

---